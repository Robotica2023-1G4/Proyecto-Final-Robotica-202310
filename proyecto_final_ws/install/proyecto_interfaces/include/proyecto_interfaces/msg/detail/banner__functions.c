  ...
  proyecto_interfaces__msg__Banner__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    proyecto_interfaces__msg__Banner * data =
      (proyecto_interfaces__msg__Banner *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!proyecto_interfaces__msg__Banner__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          proyecto_interfaces__msg__Banner__fini(&data[i]);
        }
        free(data);
        return false;
      }
